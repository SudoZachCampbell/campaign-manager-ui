        ...
        <Box mb={!props.noEdit && 2} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} display="flex">
            {!props.noEdit || props.column ? (
                <Box display='flex' mr={3} flexDirection={props.column ? 'column' : 'row'}>
                    <Typography variant={props.title ? 'h6' : 'subtitle2'} style={{ marginRight: '1em', marginBottom: '0.25em' }}> {props.label}:</Typography>
                    <Box display='flex'>
                        {props.children}
                    ...
                    <Grid container xs={12}>
                        <Grid item xs={5}>
                            <Typography variant={props.title ? 'h6' : 'subtitle2'} style={{ marginRight: '1em' }}> {props.label}:</Typography>
                        </Grid>
                        <Grid item xs={7}>
                            <Box display='flex'>
                                {props.children}
